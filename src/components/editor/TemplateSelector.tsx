...
import { Check, Crown, Globe, SlidersHorizontal } from "lucide-react";
...
const TemplateCard = React.memo(function TemplateCard({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "group relative flex flex-col gap-2 cursor-pointer select-none active:scale-95 transition-all duration-200 rounded-xl focus:outline-none w-full text-left",
...
export const TemplateSelector = React.memo(function TemplateSelector({ onSelect }: { onSelect?: () => void }) {
  ...
  // Defer filters to optimize performance and prevent input lag
  const deferredLangFilter = React.useDeferredValue(langFilter);
  const deferredPriceFilter = React.useDeferredValue(priceFilter);
  ...
  const filteredTemplates = React.useMemo(() => {
    ...
    // Apply price filter
    if (deferredPriceFilter === "free") {
      result = result.filter((t) => !t.isPremium);
    } else if (deferredPriceFilter === "premium") {
      result = result.filter((t) => t.isPremium);
    }
